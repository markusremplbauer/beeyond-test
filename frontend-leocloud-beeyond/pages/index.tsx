...
// const url = `${publicRuntimeConfig.API_URL}/template`;
...
export default function Home({
  ...
  const { data } = useSWR<Template[]>(`${backendUrl}/template`, fetcher);
  // const { data } = useSWR<Template[]>(`${backendUrl}/template`, fetcher);
  const [setApiURL, setBasePath] = useEnvStore((state) => [
    state.setApiURL,
    state.setBasePath,
  ]);
...
Home.getInitialProps = async () => {
  // const backendUrl = publicRuntimeConfig.API_URL;
  return {
    backendUrl: global.process.env.API_URL,
